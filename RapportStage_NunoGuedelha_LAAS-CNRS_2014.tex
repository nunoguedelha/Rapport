...
Dans les sections précédentes, nous avons posé les bases de l'algèbre spatiale des torseurs cinématiques et dynamiques, ainsi que la modélisation généralisée d'un arbre cinématique (\emph{modèle de sytème} du robot). Nous allons décrire dans cette section, la définition et l'implémentation de l'\emph{algorithme hybride}, qui se repose sur deux phases:
\begin{itemize}
\item Les modifications apportées aux algorithmes \gls{acr_rneaLabel} et \gls{acr_crbaLabel} et leur intégration dans la construction de l'algorithme principal
\item L'optimisation logique visant à réduire le nombre d'étapes de calcul, ainsi que l'optimisation de la compilation et exécution de ces étapes (\emph{Meta-programmation})
\end{itemize}


\section{l'algorithme hybride en quatre étapes}

Nous considérons l'équation de mouvement relative à un arbre cinématique générique (à base fixe ou flottante), sous la forme matricielle comme suit:
...
\begin{Parallel}[c]{.5\textwidth}{.5\textwidth}
\ParallelLText{%
